...
public enum TaskDueDate: String {
  ...
  public func isOverdueForMoveDate(moveDate: NSDate) -> Bool {
	let taskDueDate = dueDateForMoveDate(moveDate: moveDate)
    
	return taskDueDate.compare(NSDate.startOfToday() as Date) == ComparisonResult.orderedAscending
  }
  ...
  func dueDateForMoveDate(moveDate: NSDate) -> NSDate {
	...
	return NSCalendar.current.date(byAdding: components as DateComponents, to: moveDate as Date) as! NSDate
  }
  
  public func daysFromDueDate(moveDate: NSDate) -> String {
	let taskDueDate = dueDateForMoveDate(moveDate: moveDate)
	
	let components = NSCalendar.current.dateComponents([.day], from: NSDate.startOfToday() as Date, to: taskDueDate as Date)
